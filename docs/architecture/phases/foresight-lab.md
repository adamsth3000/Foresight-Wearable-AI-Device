...
- an optional openWakeWord-based hands-free wake adapter that emits a minimal event for the fixed `Hey Foresight` phrase from a developer-provided ONNX model
- a separate wake-training workspace with versioned prototype and quality profiles, resumable stage manifests, and no trained model artifact

The wake adapter does not interpret commands and the microphone adapter does not own wake handling, intent interpretation, pending context, or session logic. In hands-free mode, the wake adapter waits for the fixed phrase, releases microphone ownership, and the existing bounded voice flow captures a command plus any required follow-up before returning to idle wake listening. The typed `voice` command remains a Lab development control. The wake phrase is intentionally separate from ordinary intent interpretation. The current phrase rules are demonstrations for Lab testing, not a permanent language model or an exhaustive command vocabulary. The architecture leaves room for a separate future priority-input path, but no emergency path is implemented.
...
This implemented scope is still intentionally limited. It does not include continuous full-speech transcription, production wake-word detection, streaming or background audio, phone/wearable/Bluetooth/bone-conduction output, haptics, gesture recognition, media ingestion, GPS integration, replay, persistence, AI or NLP systems, or hardware adapters beyond the optional Lab voice, wake, and output adapters.

The training workspace does not run training automatically, download quality datasets by default, or change the main Foresight runtime environment. A future ONNX model remains developer-local until explicitly evaluated and deployed.
