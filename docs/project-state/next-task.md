...
Define the next implementation milestone after `Foresight Lab v0.4` so the Lab can exercise a further narrow workflow without introducing continuous capture or hardware-specific core behavior.

## Why This Is Next
- A normalized interaction, session, terminal command, wake, multi-step context, and optional microphone transcript flow now exists.
